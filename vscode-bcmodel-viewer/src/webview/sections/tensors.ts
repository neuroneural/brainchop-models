...
export function renderTensors(parsed: ParsedBcmodel): string {
  ...
  const rows = parsed.tensorStats
    .map((t, i) => `<tr data-idx="${i}" data-name="${t.name.toLowerCase()}">
      ...
      <td class="num">${formatBytes(t.sizeBytes)}</td>
  ...
  return `
  <section class="card">
    <h2>Weight Tensors <span class="count">${parsed.tensorStats.length} tensors &middot; ${parsed.totalParams.toLocaleString()} params &middot; ${formatBytes(parsed.totalWeightBytes)}</span></h2>

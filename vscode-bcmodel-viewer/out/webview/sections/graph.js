...
const dagLayout_1 = require("../dagLayout");
...
function renderNodeSvg(node) {
    ...
    const paramsJson = (0, utils_1.escapeHtml)(JSON.stringify(node.params));
    return `<g class="dag-node" data-id="${(0, utils_1.escapeHtml)(node.id)}" data-op="${(0, utils_1.escapeHtml)(node.op)}" data-params="${paramsJson}" data-inputs="${(0, utils_1.escapeHtml)(node.inputs.join(','))}">
    ...
    <text x="${node.x + 8}" y="${node.y + node.height / 2 + 1}" dominant-baseline="middle"
      ...
      <tspan font-weight="600">${(0, utils_1.escapeHtml)(shortLabel)}</tspan>
      <tspan fill="var(--vscode-descriptionForeground)" font-size="10"> ${(0, utils_1.escapeHtml)(idLabel)}</tspan>
    </text>
  </g>`;
}
function renderGraph(parsed) {
    ...
    const layout = (0, dagLayout_1.computeDagLayout)(graph);
    // Count unique ops

...
const inference_1 = require("./sections/inference");
const labels_1 = require("./sections/labels");
const tensors_1 = require("./sections/tensors");
const graph_1 = require("./sections/graph");
function buildWebviewHtml(parsed) {
    const nonce = (0, utils_1.getNonce)();
    const name = (0, utils_1.escapeHtml)(parsed.header.metadata.name);
    const version = (0, utils_1.escapeHtml)(parsed.header.bcmodel_version);
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style nonce="${nonce}">${(0, theme_1.getBaseStyles)()}</style>
</head>
<body>
  <div class="container">
    <h1 class="title">${name}<span class="badge">bcmodel v${version}</span></h1>
    <div class="subtitle">
      ${(0, utils_1.formatBytes)(parsed.fileSizeBytes)} &middot;
      ${parsed.totalParams.toLocaleString()} parameters &middot;
      ${parsed.header.graph.length} graph nodes
    </div>

    ${(0, metadata_1.renderMetadata)(parsed)}
    ${(0, inference_1.renderInference)(parsed)}
    ${(0, graph_1.renderGraph)(parsed)}
    ${(0, tensors_1.renderTensors)(parsed)}
    ${(0, labels_1.renderLabels)(parsed)}
  </div>
